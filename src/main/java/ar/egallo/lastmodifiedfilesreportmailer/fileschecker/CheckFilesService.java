...
public class CheckFilesService {
     ...
     */
	public List<File> checkFiles(final String path, final Instant filesFrom, final Instant filesTo) {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
		  if (listOfFiles[i].isFile()) {
			  ...
			  if (filesFrom.toEpochMilli() <= lastModified && filesTo.toEpochMilli() >= lastModified) {
			      final SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
				  logger.info("File " + listOfFiles[i].getAbsolutePath() + " / " + sdf.format(lastModified));
				  result.add(listOfFiles[i]);
			  }
		  } else if (listOfFiles[i].isDirectory()) {
		    result.addAll(checkFiles(path+"/"+listOfFiles[i].getName(),filesFrom, filesTo));
		  }
		}
		return result;
	}
